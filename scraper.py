...
entity_id = "NFTRW6_PAHNFT_gov"
url = "http://www.pat.nhs.uk/quality-and-performance/transparency-information.htm"
errors = 0
data = []
...
for link in year_links:
    ...
    if 'http://' in link['href']:
        year_link = link['href']
    if 'http://' not in link['href']:
        year_link = 'http://www.pat.nhs.uk'+link['href']
    ...
    for link in links:
        if '.csv' in link['href'] or '.xls' in link['href'] or '.xlsx' in link['href'] or '.pdf' in link['href']:
            if 'http://' not in link['href']:
                url = 'http://www.pat.nhs.uk'+link['href']
            if 'http://' in link['href']:
                url = link['href']
